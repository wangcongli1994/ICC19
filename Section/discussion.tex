...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Firefox certificate management cannot add self-signed endpoint certificate}
For a self-signed endpoint certificate, it is both a root certificate and a CA certificate. This type of certificate can be added to the "Trusted Root Certification Authorities" store on Windows systems and made available to Chrome and Edge browsers for secure access. Firefox has a self-built certificate manager, but the manager does not store the endpoint certificate in the certificate store. That is, if you need Firefox to trust a self-signed certificate, you must sign a endpoint certificate by using the CA certificate, and install the root CA certificate into the certificate manager of the Firefox browser.
