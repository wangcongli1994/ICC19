...
If the HSTS policy is enforced by the pre-loaded list, which is explicitly configured by the user,
    the user shall be notified on such visits over HTTP.
If the HSTS policy is enforced  by the HSTS header, which is sent by the website,
