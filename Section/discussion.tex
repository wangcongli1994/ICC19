...
%        we think Chrome need to be improved on this.
If a revoked certificate whose private key is stolen and controlled by attacker 
    is not included in CRLSets,
    Chrome users are vulnerable to impersonation attacks until the certificate expires.
...
However,
    ...
    i.e., an entity holds a certificate where the cA flag is false, and acts as CAs to sign server certificates
        for any domain names to launch MitM attacks,
...
%    in our opinion,
    ...
    and it is necessary for complete mismatch to trigger a higher-level warning.
