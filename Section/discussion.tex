...
%这一部分有待做实验确认一下，Firefox对自签名证书的机制貌似没这么简单

%对于不公开的内部网，这不是问题，此时你可以忽略该警告信息。
  % 自签名的证书虽然可以保证您不被第三方窃听，但无法知晓数据的接收者是谁。这对处于内部局域网且无法在外网访问的网站来说很常见，您可以选择绕过警示。
  %  Self-signed server certificate cannot be added to NSS certificate store,
  %  whenever Firefox encounters a self-signed certificate, it will dispay a B-level (Medium-risk) warning.
   ...
   % The only way is to add an exception, but the memory time for the exception is limited
