...
In this section,
    we emphasize the defects that may lead to successful MitM or impersonation attacks.
...
Chrome checks the revocation status
    only through CRLSet \cite{CRLSet}, which covers only 0.35\% of all revocations in the Internet \cite{liu2015end}.
...
So a compromised key pair can still be exploited to launch MitM or impersonation attacks after the certificate is revoked through CRL and OCSP,
        and the attacks will probably succeed in Chrome.
...
These defects may be exploited to launch successful MitM and impersonation attacks.
...
However, in our opinion,
    such configurations are rather uncommon cases,
...
%    in our opinion,
    ...
    and it is practicable to show a higher-level warning on complete mismatch because low-cost TLS server certificates are available.
...
Firefox, Edge, and IE consider a server certificate as secure,
    if it contains the valid CN field but no SAN;
