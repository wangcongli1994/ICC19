...
The first factorization of 512-bit RSA was finished in 1999 \cite{cavallar2000factorization},
    and an 512-bit RSA key pair was broken in 73 days by a desktop computer in 2009 \cite{rsa512broken}.
...
However,
    ...
    i.e., a non-CA certificate holder acts as CAs to sign server certificates,
...
The defects may be exploited to launch successful MitM attacks.
...
However, in our opinion,
    such configurations are not common cases,
    a Level-A warning shall be necessary.

\textbf{Extended Key Usage and Key Usage.}
If a certificate contains the extensions of both Key Usage and Extended Key Usage,
    the extensions must be processed independently,
...
This rule is not complied with in Chrome, Edge and IE:
    when the server certificate includes only the serverAuth extended usage and the keyCertSign usage,
     no warning is shown.
...
However,
        Chrome and Firefox do not conduct HPKP validation
            when the certificate chain is verified by a root CA certificate installed by users.
So a website cannot enhance its HTTPS security by HPKP against MitM attacks,
...
If the HSTS policy is activated by the pre-loaded list, which is explicitly configured by the user,
    the user shall be notified on such visits over HTTP.
If the HSTS policy is activated by the HSTS header, which is sent by the website,
    such visits over plain HTTP are probably initiated by attackers because the website does not allow HTTP visits.
