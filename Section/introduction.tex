...
These security warnings try to protect users to establish secure connections with the target website,
    but not a man-in-the-middle (MitM) attack.
However, an error does not definitely result from the MitM attackers.
...
Our study
    discloses browser defects on handling HTTPS errors in terms of cryptographic algorithm,
     certificate extension, name validation,
      certificate revocation, and HPKP.
Some defects cause a forged or revoked certificate to be probably accepted by users:
\begin{itemize}
\item
A certificate signed or forged with weak cryptographic algorithms is accepted without warnings.
Edge and IE consider MD5 and SHA-1 as secure,
    and all these browsers accept RSA-1024 without any warning.
...
\textcolor[rgb]{1.00,0.00,0.00}{Chrome, Firefox, Edge and IE do not process the extensions of Key Usage and Extended Key Usage correctly.
...
The security-enhancement of HPKP does not take effect always against MitM attacks.
...
\end{itemize}
There are also defects leading to the problems of usability.
%\begin{itemize}
%\item
Chrome implements the procedure of name validation,
    different from other browsers,
    only with the field of commonName (or called CN) but not the extension of Subject Alternative Name (SAN).
...
We finally present the suggestions to improve the browsers.
