...
Recently,
    the security of HTTPS is further enhanced by HPKP that pins the public key (or certificate) of the website in browsers \cite{Evans2015rfc7469},
...
However, an error does not definitely result from the attackers.
...
Some defects may cause a forged or revoked certificate to be probably accepted by users:
\begin{itemize}
\item
A certificate signed with weak cryptographic algorithms is accepted without warnings.
Edge and IE consider MD5 and SHA-1 as secure,
    and all these browsers accepted RSA-1024 without any warning.
...
\textcolor[rgb]{1.00,0.00,0.00}{Chrome, Edge and IE do not process the certificate extensions of Key Usage and Extended Key Usage correctly:
even when these extensions appear inconsistently in certificates,
    no warning shows.}   % 总结不够到位
...
\item
Chrome does not check the revocation status of server certificates through commonly-used CRL and OCSP.
\item
The HPKP support of Chrome and Firefox
    does not work well,
    if the server certificate is verified by a user-installed root CA certificate.
...
Chrome implements the procedure of name validation,
    ...
    with the field of commonName and the extension of subject alternative name.
A server certificate considered as secure by Firefox, Edge and IE,
