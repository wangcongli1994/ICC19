...
The security warnings try to protect users to establish secure connections with the target website,
    but not a man-in-the-middle (MitM) attack.
However, an error does not definitely result from the MitM attacks.
In order to balance security and usability,
...
Our study
    discloses browser defects on handling HTTPS errors in terms of cryptographic algorithm,
     certificate extension, certificate revocation, name validation,
      and HPKP.
Some defects cause a forged or revoked certificate to be probably accepted by users:
\begin{itemize}
\item
...
Meanwhile, all browsers accept server certificates binding an RSA-1024 key pair without any warning,
        which prohibited by CA/Browser Forum since 2014 \cite{CAB2018BR},
            and by NIST since 2016 \cite{NIST2005ECC}.
...
Chrome, Firefox, Edge and IE do not process the extensions of Key Usage and Extended Key Usage correctly.
...
In all browsers,
    the visits over plain HTTP that violates the HSTS policy,
        cause only the low-risk warnings, not interrupting the user's browsing.
...
However,
    such visits deviate from the user's requirements or probably result from the MitM attacks.
\end{itemize}
...
So a legitimate server certificate considered as secure by Firefox, Edge and IE,
    may cause a security warning in Chrome.
%Each browser has
