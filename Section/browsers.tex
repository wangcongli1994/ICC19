...
The  attacks might be launched
    ...
    or deceiving browsers to visit the website through HTTP.
...
On low-risk errors,
 the browser still establishes HTTPS connections with the visited website.
...
For medium-risk errors, it shows a \emph{bypassable warning} that discourages the user from continuing.
For example,
\textcolor[rgb]{1.00,0.00,0.00}{    the browser stops the webpage load and displays a full-screen warning with two options: ``Click here to close this webpage'' and ``Continue to this website (not recommended)''}. % 与图片不同？ why
On medium-risk errors,
    if the user bypasses (or clicks through) the warning,
     HTTPS connections are then established between the browser and the web server.
...
For high-risk errors, it shows a \emph{non-bypassable warning} that stops the browsing.
For example,
    the browser displays a full-screen web page explaining the error,
     with a retry button.
On high-risk errors,
...
For fatal errors, it \emph{closes the connections} and stops the browsing.
For example,
