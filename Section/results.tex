...
\multirow{17}{*}{\begin{tabular}[c]{@{}c@{}}Certificate Verification\\of Extensions\end{tabular}}
 & Server certificate &  &  &  &  \\
 & ~~incorrect cA flag (= true), correct/no Key Usage and Extended Key Usage & Secure & B & Secure & Secure \\
...
In these tests,
    the certificate chains consist of one self-signed root CA certificate, an intermediate CA certificate, and the server certificate.
...
If the cA flag of the Basic Constraints extension is true in the ``server'' certificate
    but the key usage is set as a common TLS server certificate (i.e.,
...
Chrome, Edge, and IE show Level-C warnings,
     and Firefox shows a bypassable Level-B warning.
...
Chrome and Edge show Level-C high-risk warnings, IE shows a Level-B warning,
    but Firefox consider it as secure.
...
If the server certificate is signed without the SAN extension
    and the CN field is set incorrect, all browsers show Level-B warnings.
If it is signed without SAN
...
When we visit a website in the HSTS pre-loaded list,
    if there is an HTTPS error that triggers Level-B warnings without HSTS (for example,
        ...
        complete mismatch, WWW mismatch, out-of-wildcard-scope subdomain, 
        ...
        \textcolor[rgb]{1.00,0.00,0.00}{or any other Level-B medium-risk HTTPS errors}),  %% 具体哪些，列出来；包括Level-D的错误吗？不包括Level-D的错误
...
All browsers show Level-A warnings if some resources are visited over HTTP.
