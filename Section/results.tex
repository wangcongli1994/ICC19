...
% & Untrusted self-signed server certificate & B & \textcolor[rgb]{1.00,0.00,0.00}{B[xx]} & B & B \\     % 这里应该有点什么专门的解释说明
 ...
 & Weak key (512-bit RSA) & B & B & B & C \\
 & Weak key (1024-bit RSA) & Secure & Secure & Secure & Secure \\
 & Weak key (160-bit ECC) & D & D & D & D \\ \cline{2-6}
...
the Extended Key Usage extension is non-critical.
...
Next, the CN field is set different from the SAN extension,
...
In these tests,
    the root CA certificate is (\emph{a}) generated by OpenSSL and then manually installed in the browsers' certificate store,
            i.e., in the private store,
    or (\emph{b}) trusted in browsers by default, i.e., in the public store.
Let's Encrypt in the trusted root CA in these tests.
...
An incorrect HPKP header is sent when the evaluated browser visits for the first time,
 such as non-numeric max-age and pin-sha256 mismatch.
No warning on HPKP errors is shown,
        when the browser visits for the second time.
A correct HPKP header is sent,
when the evaluated browser visits for the first time.
Then,
...
When visiting a domain in the list,
    the browser only uses HTTPS to initiate the connections.
...
When we visit a website in the HSTS pre-loaded list, 
    if there is an HTTPS error that causes Level-B warnings without HSTS (for example,
        incomplete certificate chain, expired certificate, \textcolor[rgb]{1.00,0.00,0.00}{or any other Level-B medium-risk HTTPS errors}),  %% 具体哪些，列出来
