...
We register a domain name and
    establish the website by Nginx.
...
The browsers visit  the ``honeypot'' website,
...
Level-D warnings are shown when it is impossible to establish HTTPS connections; 
    for example,
    the cryptographic algorithm in certificates is unsupported.
...
2048-bit RSA and SHA-256 are used in these tests,
    except for the tests of weak key pair and weak hash algorithm.      %% 这句话描述，对不对？  对！
...
\toprule
\multicolumn{2}{c|}{\multirow{2}{*}{HTTPS Error}} & \multicolumn{4}{l}{Browser Security Warning} \\ \cline{3-6}
...
In these tests,
    the certificate extensions of Basic Constraints, Key Usage, Extended Key Usage and SAN,
        are correctly set.
The domain name in CN also appears to be correct.
...
(\emph{b}) CRL. When it is revoked through CRL,
 Chrome and Firefox consider it as secure, while Edge and IE show Level-C warnings \cite{liu2015end}.  %%% CRL实验，应该也是我们自己做。
...
%In this weak key paragraph, all server certificates are signed by SHA-256 with RSA Encryption.
 When receiving a server certificate binding a 512-bit RSA key pair,
    Chrome, Firefox, and Edge show Level-B warnings,
            while a Level-C warning is shown in IE.
...
In these tests of certificate extensions,
    the certificate chains consist of one self-signed root CA certificate, an intermediate certificate, and the server certificate.
...
If the cA flag is true in the server certificate
    and the key usage is set as a common CA certificate (i.e.,
...
Therefore, even if an entity holds a certificate where the cA flag is false,
 it can issue certificates for any domain names to launch successful MitM attacks, especially in Firefox,
 because Firefox shows only \emph{bypassable} Level-B medium-risk warnings.

If the pathLenConstraint field of the Basic Constraints extension
    in the root CA certificate is 0
    and the cA flag is true in the intermediate CA certificate (i.e.,
     the pathLenConstraint field is violated in the certificate chain),
Chrome and Edge show Level-C high-risk warnings,
    Firefox consider it as secure, and IE shows a Level-B warning.


\textbf{Key Usage and Extended Key Usage.}
%    We didn't designate key usage and the extend key usage as critical or non-critical.
As long as the server certificate is signed with the Extended Key Usage of serverAuth,
...
When the server certificates is signed without the Extended Key Usage extension,
    ...
    while other browsers always consider it as secure.
...
which is marked as non-critical.

Next, the CN field is set different from the non-critical SAN extension,
...
If the server certificate signed without the SAN extension
    and the CN field is set incorrect, all browsers show Level-B warnings.
If the server certificate signed without the SAN extension
    and the CN field is correct,
Firefox, Edge, and IE consider it as secure
   but Chrome shows a Level-B warning.

\subsection{HTTPS Security-Enhancement Errors}
\textbf{HPKP.}
In these tests,
    the root CA certificate is (\emph{a}) generated by OpenSSL and then manually installed in the browsers' certificate store,
            i.e., in the private certificate store,
    or (\emph{b}) trusted in browsers by default, i.e., in the public store.
Let's Encrypt is the publicly-trusted root CA in these tests.

Edge and IE do not support HPKP, so they do not show any warnings on HPKP errors.
Chrome and Firefox exhibit the same behaviors on HPKP validation.
%
...
It seems that HPKP validation is not activated.

When the root CA certificate is in the public store, there are two categories of tests.
...
(\emph{b}) A correct HPKP header is sent,
when the browser visits for the first time.
Then,
    when the browser visits the website for the second time,
        ...
        no certificate in the certificate chain matches the pinned public key).
...
\textbf{HSTS.}
HSTS follows the trust-on-first-use policy, and it also supports the pre-loaded list in browsers \cite{hodges2012rfc6797}.
When visiting a domain in the list,
    the browser must use HTTPS. % to initiate the connections.
...
When we visit a website in the HSTS pre-loaded list,
    if there is an HTTPS error that triggers Level-B warnings without HSTS (for example,
        incomplete certificate chain, expired certificate, \textcolor[rgb]{1.00,0.00,0.00}{or any other Level-B medium-risk HTTPS errors}),  %% 具体哪些，列出来；包括Level-D的错误吗？
...
All browsers show Level-A warnings if some resources are visited over plain HTTP.

After the visit with a correct HSTS header,
