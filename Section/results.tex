...
We use OpenSSL to generate a self-signed root certificate(whose cA flag is true, pathLenConstraint is null, key usage is keyCertSign), 
...
This warning can be avoided by installing the self-signed root certificate to the browsers' root certificate store
(Chrome, Edge, IE use OS root certificate store as browser root certificate store, and Firefox has its own NSS root certificate store).
...
Other errors including incomplete certificate chain and expired certificate,
        all browsers show Level-B medium-risk warnings (The self-signed root certificate has been installed to the browsers' trust root certificate store).
    



\textbf{Revoked certificate.}
We only consider the case where the server certificate is revoked and all CA certificates are not revoked.
(\emph{a}) OCSP. We get a certificate for our website’s domain from Let’s Encrypt, and then revoked it.
                Let’s Encrypt only distributes revocation status through OCSP.
                When visiting this website, Chrome considers it secure, but Firefox, Edge, and IE display Level-C high-risk warnings.
(\emph{b}) CRL. If a server certificate is only revoked through CRL, Chrome and Firefox consider it secure, but Edge and IE display Level-C high-risk warnings \cite{liu2015end}.
(\emph{c})CRLset. If a revoked server certificate is included in CRLset, Chrome will displays a Level-C high-risk warning.
...
In this weak key paragraph, all server certificates are signed by SHA-256 with RSA Encryption.
 When receiving a server certificate containing a 512-bit RSA key, 
    Chrome, Firefox, and Edge display Level-B medium-risk warnings,
            but IE display a C-level (High-risk) warning. 
 When receiving a server certificate containing a 1024-bit RSA key,
    all four browsers consider it secure.
 When receiving a server certificate containing a 160-bit ECC key,
    all four browsers display Level-D fatal warnings.
...
In this weak hash algorithm paragraph, all server certificates are signed by RSA and the public key of server certificates are 1024-bit RSA key.
    When receiving a server certificate signed by SHA-1 or MD5, 
        Chrome and Firefox display Level-B medium-risk warnings,
            but Edge and IE consider it secure.
...
If the root certificate of the certificate chain is installed manually by the user, no HPKP errors will be warned.
 ...
 \textbf{HSTS}
 HSTS preload list is a list of domain 
