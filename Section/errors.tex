...
the errors of certificate verification of basic fields and extensions,
 name validation, and also security-enhancement of HTTPS.
...
The server certificate binds a key pair of weak algorithm,
    such as RSA-512/1024 \cite{CAB2018BR}, or ECC-160 \cite{NIST2005ECC}.
...
The following certificate extensions are important
...
The pathLenConstraint integer must appear in a CA certificate,
    and
    it gives
        the maximum number of intermediate CA certificates following this CA certificate in a valid certificate chain.
...
A TLS server certificate must include the serverAuth extended key usage \cite{cooper2008rfc5280}.
...
Browsers check
    ...
    through the CN field and the SAN extension.
%    Name validation error means that domain names listed in a server certificate don't match the domain we visited.
There following errors of name validation may happen in CN and/or SAN.
...
%    or there is any error in the HSTS header.
     \textcolor[rgb]{0.00,0.07,1.00}{there is any HTTPS error in an HSTS website.}
